...
export function parsePresetFromJson(text: string, fallbackName: string = "导入的预设"): PresetConfig | null {
    try {
        ...
        if (Array.isArray(obj.enabled_generation_parameters)) {
            const enabledParameters: GenerationParameterKey[] = [];
            for (const value of obj.enabled_generation_parameters as unknown[]) {
                if (isGenerationParameterKey(value)) enabledParameters.push(value);
            }
            preset.enabled_generation_parameters = [...new Set(enabledParameters)];
        }
        // New preset globals
